if global.count_to_exit == 7 {
	if dialog == 1 then draw_dialog(1, "Привет!", woman_sound, 1, empty, "Девушка");
	...
	if loop_past == 0 {	
		if global.math_exam == 0 {
			...
			if global.skip_prolog == 1 {
				if dialog == 3 then draw_dialog(1, "И это слышали.", boy_sound, 1, narrator_smile, "???");
				if dialog == 4 {
			...
			if dialog == 2 then draw_dialog(1, "Небось весь семестр дома отсиживался?", woman_sound, 1, empty, "Девушка");
				if global.skip_prolog == 1 {
				if dialog == 3 then draw_dialog(1, "Мы это уже слышали.", boy_sound, 1, narrator_smile, "???");
				if dialog == 4 {
					global.audit_check += 1;
					global.player_stop = false;
					alarm[0] = room_speed;
